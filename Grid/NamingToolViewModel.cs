...
using FlewsApp.Modules.DataStore.Extensions;
...
public class NamingToolViewModel : FlewsDialogViewModelBase
{
  ...
  private void InitialiseDataLibItems()
  {
    ...
    if (_dataManager.Globals!.Platforms.Any() || _dataManager.Globals!.Weapons.Missiles.Any() || _dataManager.Globals.Systems.AllSystems.Any())
    {
      ...
      ItemNamingRecords.AddRange(_dataManager.Globals!.Platforms.FilteredByMaxClassification(_dataManager).Select(s => new PlatformItemNamingRecord(s)));
      ItemNamingRecords.AddRange(_dataManager.Globals.Weapons.Missiles.FilteredByMaxClassification(_dataManager).Select(s => new MissileItemNamingRecord(s)));
      ItemNamingRecords.AddRange(_dataManager.Globals.Systems.AllSystems.FilteredByMaxClassification(_dataManager).Select(s => new SensorItemNamingRecord(s)));
  ...
  private void GlobalsWatcher_CollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
  {
    ...
    if (e.NewItems != null)
    {
      foreach (var item in e.NewItems.OfType<StoredObject>().FilteredByMaxClassification(_dataManager))
      {
